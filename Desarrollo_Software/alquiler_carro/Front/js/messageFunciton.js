    ...
    $.ajax({
      ...
      success: function (respuesta) {
        ...
        $.each(respuesta, function (_id, reservationInfo) {
          $select.append(
            '<option value=' +
              reservationInfo.idReservation +
              '>' +
              formatDate(new Date(reservationInfo.startDate), 1) +
              ' - ' +
              formatDate(new Date(reservationInfo.devolutionDate), 1) +
              '</option>'
          )
        })
      },
...
function drawMessage (messages) {
  ...
  for (const element of messages) {
    ...
    tableessages +=
      "<td> <button onclick='launchDataMessage(" +
      element.idMessage +
      ")'>Lanzar</button>"
    tableessages +=
      "<td> <button onclick=' putMessage(" +
      element.idMessage +
      ")'>Actualizar</button>"
    tableessages +=
